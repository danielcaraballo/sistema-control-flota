...
- **Seed data**: `vehiculos/seed_data.py` is the source of truth for catalog/organization reference data (24 estados, 44 gerencias, 982 centros, 111 marcas, 520 modelos, 13 colores, 8 estatus, etc.). Migrations `catalogos.0007_seed_catalog_data` and `organizacion.0007_seed_org_data` read this module and `update_or_create` records by natural keys. DO NOT edit this file by hand for minor corrections — update via the admin UI instead. For bulk regeneration, re-extract from the CSV source via a new migration.
...
6. **Seed data (opcional):** Agregar valores por defecto en `backend/vehiculos/seed_data.py` y una migración de datos que haga `update_or_create` leyendo de ese módulo.
